...
if exist([ save_name '.mat'])==2,
    ...
    if exist('copyfile'),
        ...
        while cont,
            ...
            save_nameOld = save_name;
            save_name = [ save_name postfix];
            cont = (exist([ save_name '.mat'])==2);
        end;
        copyfile([save_nameOld '.mat'],[save_name '.mat']);
        disp(['Copying the current ' save_nameOld '.mat file to ' save_name '.mat']);
        if exist('Calib_Results.m')==2,
            copyfile([save_nameOld '.m'],[save_name '.m']);
            disp(['Copying the current ' save_nameOld '.m file to ' save_name '.m']);
        end;
        cont_save = 1;
    else
        disp(['The file ' save_nameOld '.mat is about to be changed.']);
