...
%     imwrite(uint8(I2),gray(256),'left_rect.jpg','jpg');
    ...
    I = double(imread('left.jpg'));
    %Split images in channels RGB
    rChn = I(:,:,1);
    gChn = I(:,:,2);
    bChn = I(:,:,3);
    
    rChn2 = 255.*ones(ny,nx); gChn2 = rChn2; bChn2 = rChn2;
    
    rChn2(ind_new_left) = uint8(a1_left .* rChn(ind_1_left) + a2_left .* rChn(ind_2_left) + a3_left .* rChn(ind_3_left) + a4_left .* rChn(ind_4_left));
    gChn2(ind_new_left) = uint8(a1_left .* gChn(ind_1_left) + a2_left .* gChn(ind_2_left) + a3_left .* gChn(ind_3_left) + a4_left .* gChn(ind_4_left));
    bChn2(ind_new_left) = uint8(a1_left .* bChn(ind_1_left) + a2_left .* bChn(ind_2_left) + a3_left .* bChn(ind_3_left) + a4_left .* bChn(ind_4_left));
    
    imwrite(uint8(cat(3, rChn2, gChn2, bChn2)), 'left_rect.jpg','jpg');
...
%     imwrite(uint8(I2),gray(256),'right_rect.jpg','jpg');
    ...
    rChn2(ind_new_right) = uint8(a1_right .* rChn(ind_1_right) + a2_right .* rChn(ind_2_right) + a3_right .* rChn(ind_3_right) + a4_right .* rChn(ind_4_right));
    gChn2(ind_new_right) = uint8(a1_right .* gChn(ind_1_right) + a2_right .* gChn(ind_2_right) + a3_right .* gChn(ind_3_right) + a4_right .* gChn(ind_4_right));
    bChn2(ind_new_right) = uint8(a1_right .* bChn(ind_1_right) + a2_right .* bChn(ind_2_right) + a3_right .* bChn(ind_3_right) + a4_right .* bChn(ind_4_right));
    
    imwrite(uint8(cat(3, rChn2, gChn2, bChn2)), 'right_rect.jpg','jpg');
...
if ~isempty(calib_name_left)&~isempty(calib_name_right),
    ...
    for kk = find(active_images);
        ...
        [I, Irgb] = load_image(kk,calib_name_left,format_image_left,type_numbering_left,image_numbers_left,N_slots_left);
        
        fprintf(1,'Image warping...\n');
        
        
        if(blackAndWhiteRect || size(Irgb,3) < 3)
            I2 = 255*ones(ny,nx);
            I2(ind_new_left) = uint8(a1_left .* I(ind_1_left) + a2_left .* I(ind_2_left) + a3_left .* I(ind_3_left) + a4_left .* I(ind_4_left));
        else
            rChn = Irgb(:,:,1);
        ...
        [I, Irgb] = load_image(kk,calib_name_right,format_image_right,type_numbering_right,image_numbers_right,N_slots_right);
        
        fprintf(1,'Image warping...\n');
        if(blackAndWhiteRect)
            I2 = 255*ones(ny,nx);
            I2(ind_new_right) = uint8(a1_right .* I(ind_1_right) + a2_right .* I(ind_2_right) + a3_right .* I(ind_3_right) + a4_right .* I(ind_4_right));
        else
            rChn = Irgb(:,:,1);
